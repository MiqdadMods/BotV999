...
/*****************************************/
/********** OWNER NUMBER**********/
const ownerNumber = ["6285326364468@s.whatsapp.net","6285326364468@s.whatsapp.net"] 
/*************************************/ 
/*********** LOAD FILE ************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('©','red'),color(']','white'),color('SCAN QR CODE !','white'),color('SUBSCRIBE','yellow'),color('𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨','aqua'))
})
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const 𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨 = " ~ by 𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨"
		...
		case 'nulis2': 				
				...
				reply(mess.wait)
					...
					if (vuss !== undefined) {
						costum(await getBuffer(anu), image, 𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨ganss, ` ~ Harta Tahta ${args[0]}`)
					} else {
...
break
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    ...
                    client.sendMessage(from, buffer, image, {caption: 'Jangan Cuma Make Doang Follow instagram.com/𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨', quoted: mek})
         break
         case 'gplaybutton':
         case 'splaybutton':
                if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                    if (args.length == 0) return reply(`Usage: ${prefix + command} text\nExample: ${prefix + command} 𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨`)
                    txt = args.join(" ")
                    reply('[❕] Loading')
         ...
         break
				...
				client.sendMessage(from, ''+tes+'\n\n*By : 𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨*', text, { quoted: mek })
				await limitAdd(sender)
				break
                    case 'wibu':
				    if (!isRegistered) return reply( ind.noregis())
				...
				case 'googletext':
					...
					if (args.length < 1) return reply(`Contoh : ${prefix}googletext 𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨. /gans/banget`)
					reply(ind.wait())
		 ...
		 case 'report':
                ...
                const teks1 = `*[REPORT]*\nNumber : @${nomor.split("@s.whatsapp.net")[0]}\nPesan : ${pesan}`
                    ...
                    client.sendMessage('6285326364468@s.whatsapp.net', options, text, {quoted: mek})
     ...
     case 'request':
					...
					client.sendMessage('6285326364468@s.whatsapp.net', options, text, tescuk, vz, {quoted: mek})
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER 𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨 Request Palsu Atau Main² Tydak Akan Ditanggapi Terimakasih.')
					break
				case 'listhargadmff':
				case 'listdiamondff':
				if (!isRegistered) return reply(ind.noregis())
					client.sendMessage(from, listhargadmff(), text, tescuk, cr,)
					break
				case 'donasi':
				case 'donate':
				if (!isRegistered) return reply(ind.noregis())
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'Jangan Lupa Di Follow yaa : instagram.com/𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨',MessageType.text, { quoted: mek} )
		...
		if (!isGroup) return reply(ind.groupo())
               ...
               case 'tercantik':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					case 'Bot':
					case 'bot':
					client.sendMessage(from, 'Iya sayang? Ada apa manggil bot\nKangenkah? :v',MessageType.text, { quoted: mek} )
					break
					case 'Sayang':
					case 'sayang':
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'glitchtext':
					...
					var gli = gh.split("/")[0];
					var tch = gh.split("/")[1];
					if (args.length < 1) return reply(`Contoh : ${prefix}glitchtext 𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨/gans`)
					reply(ind.wait())
           ...
           case 'demote':
				...
				case 'tagall':
					...
					mentions('╔══🜲〘 OTAG ALL 〙🜲══'+teks+'╚═〘 𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨 〙', members_id, true)
					break
				case 'delete':
				case 'del':
				case 'd':
				...
				case 'clone':
					...
					var yogi = gh.split("/")[0];
					var cmnt = gh.split("/")[1];
					if (args.length < 1) return reply(`Contoh : ${prefix}phcomment 𝙈𝙞𝙦𝙙𝙖𝙙 𝙈𝙤𝙙𝙨. /yt`)
					reply(ind.wait())
